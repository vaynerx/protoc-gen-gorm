...
func (p *OrmPlugin) parseServices(file *generator.FileDescriptor) {
	...
	for _, service := range file.GetService() {
		...
		for _, method := range service.GetMethod() {
			...
			} else if strings.Contains(methodName, listService) {
				verb = listService
